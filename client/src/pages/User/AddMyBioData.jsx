...
const AddMyBioData = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: existingBioData } = await axios.get(
        `${import.meta.env.VITE_API_URL}/allBioData?email=${user.email}`
      );
  
      if (existingBioData.length > 0) {
        // If an entry with the same ContactEmail exists, show an error toast
        toast.error('A biodata with this email already exists!');
        return;
      }
  
      // Adding the biodata if the email is not found
      await axios.post(`${import.meta.env.VITE_API_URL}/allBioData`, BioData);
      toast.success('Biodata created successfully!');
      navigate('/myBioData');
    } catch (error) {
      toast.error('Failed to create biodata!', error);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 text-pink-700 py-20">
      <Helmet>
              <title>Pair Promise | Create BioData</title>
            </Helmet>
      <h1 className="text-4xl font-semibold text-center mb-8">-- Create Your Biodata --</h1>
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-6 space-y-6">
        ...
        {/* fathers name */}
        <div>
          <label htmlFor="Fathers Name" className="block text-gray-700 font-medium mb-2">
            Father's Name
          </label>
          <input
            type="text"
