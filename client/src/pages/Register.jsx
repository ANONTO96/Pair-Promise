...
import img from "../assets/hand-in-hand.png"
...
const Register = () => {
  ...
  const { signInWithGoogle, createUser, updateUserProfile, setUser } =
    ...
    const handleSignUp = async (e) => {
      ...
      try {
        ...
        if (data.insertedId) {
          ...
          toast.error('User information could not be saved');
        }
      } catch (err) {
    ...
    const handleGoogleSignIn = async () => {
      try {
        ...
        toast.error(err?.response?.data?.message || err?.message || 'Error occurred during Google Sign-In');
      }
    };
  ...
  return (
    <div className="flex justify-center items-center min-h-[calc(100vh-306px)] my-20">
      <Helmet><title>Pair Promise | Register</title></Helmet>
      ...
      style={{
        ...
        <div className="w-full px-6 py-8 md:px-8 lg:w-1/2">
          <div className="flex justify-center mx-auto">
            <img className="w-16" src="https://img.icons8.com/?size=100&id=49205&format=png&color=D61A79" alt="Matrimony Logo" />
          </div>
          ...
          <form onSubmit={handleSignUp}>
            <div className="mt-4">
              ...
              <input
                ...
                autoComplete="name"
                name="name"
                className="block w-full px-4 py-2 text-gray-700 bg-white border rounded-lg focus:border-pink-400 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-pink-300"
                type="text"
              />
            </div>
            <div className="mt-4">
            ...
            <div className="mt-6">
              <button
                type="submit"
                className="w-full px-6 py-3 text-sm font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-pink-600 rounded-lg hover:bg-pink-700 focus:outline-none focus:ring focus:ring-pink-300 focus:ring-opacity-50"
              >
                Sign Up
              </button>
            </div>
        ...
        <div
          className="hidden bg-contain bg-center lg:block lg:w-1/2"
